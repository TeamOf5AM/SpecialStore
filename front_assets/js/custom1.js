...
// jQuery('#cntctfrm').submit(function(){
//   jQuery('.field_error').html('');
//   jQuery.ajax({
//     url:'/contact_form',
//     data:jQuery('#cntctfrm').serialize(),
//     type:'post',
//     success:function(result){
  ...
  jQuery.ajax({
    ...
    success:function(result){
      ...
      if(result.status=="success"){
        ...
        jQuery('#user_button').html("<a class='nav-link' href='#' role='button' data-bs-toggle='dropdown' aria-expanded='false'><i class='fa fa-user'></i></a>");
      }
    }
  });
});
...
function solar(){
  ...
  // pv_energy = val * 1.3;
  // panel_capacity = pv_energy/4;
  // no_of_pannels = Math.round(panel_capacity/120);
  // $("#size_of_solar").html(no_of_pannels);
  // size_of_inverter
}
